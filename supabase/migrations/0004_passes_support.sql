...
-- 선불권 금액 차감 (원자적 트랜잭션, shop 소유권 검증)
...
begin
  ...
  values (p_pass_id, p_visit_id, -p_amount, '시술 차감', auth.uid());
end $$;

-- 선불권 횟수 차감 (원자적 트랜잭션, shop 소유권 검증)
...
declare
  ...
  v_shop_id uuid;
begin
  select remaining, expires_at, shop_id into v_remaining, v_expires, v_shop_id
  from passes where id = p_pass_id for update;
  ...
  insert into pass_logs (pass_id, visit_id, delta, memo, created_by)
  values (p_pass_id, p_visit_id, -1, '횟수 차감', auth.uid());
end $$;
