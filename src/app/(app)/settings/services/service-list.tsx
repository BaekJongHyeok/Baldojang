  ...
  return (
    <div className="mt-6 flex flex-col gap-5">
        ...
        {hasServices ? (
          <>
            ...
            <div className="flex items-center justify-between border-b border-border px-5 py-3">
              <p className="text-[13px] font-medium text-ink-caption">등록된 시술 {services.length}개</p>
              <button
                onClick={() => setShowAdd(true)}
                className="rounded-md bg-primary px-3 py-1.5 text-[12px] font-medium text-white transition hover:bg-primary-hover"
              >
                + 시술 추가
              </button>
            </div>
            ...
            <div className="divide-y divide-border">
              {services.map((s, idx) => (
                <div
                  key={s.id}
                  className={`flex items-center gap-3 px-5 py-3.5 ${!s.is_active ? "opacity-50" : ""}`}
                >
                  {/* 정렬 ▲▼ */}
          ...
          <div className="flex flex-col items-center px-6 py-14 text-center">
            ...
            <button onClick={() => setShowAdd(true)} className="mt-4 rounded-md bg-primary px-4 py-2 text-[13px] font-medium text-white hover:bg-primary-hover">
              + 시술 추가
            </button>
          </div>
        )}
      ...
      <div className="rounded-lg border border-border bg-white px-5 py-4">
        ...
        <p className="mt-1 text-[11px] text-ink-disabled">시술별 주기가 없을 때 적용돼요</p>
      </div>
